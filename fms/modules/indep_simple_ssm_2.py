...
class SSMCacheUnit:
    ...
    def update_conv_state(
        ...
        self.conv_state = self.conv_state + conv_state
...
class SSM(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 2. Depth‑wise convolution (no in-place ops)
        h_BC = self.act(
        ...
        H_chunks, A_chunks, B_chunks, C_chunks = [
            reshape_into_chunks(t, pad, self.chunk_size)
            for t in (H_scaled, A_scaled, B, C)
        ]

        A_perm = A_chunks.permute(0, 3, 1, 2)  # [B,H,C,L]
        A_cum = torch.cumsum(A_perm, dim=-1)
        L_tri = torch.exp(segment_sum(A_perm))

        G = (C_chunks[:, :, :, None] * B_chunks[:, :, None]).sum(-1)  # [B,C,L,L,H]
